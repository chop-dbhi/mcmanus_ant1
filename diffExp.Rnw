...
ercc.counts<-read.table("ercc.counts",col.names=c("sample","count"),stringsAsFactors=FALSE)

#these better be in the same order
stopifnot(samples==ercc.counts$sample)
sizeFactors(cds)<-ercc.counts$count

write.table(counts(cds, normalized=FALSE),file="raw_counts.tab.txt",sep="\t",quote=FALSE)
write.table(counts(cds, normalized=TRUE),file="normalized_counts.tab.txt",sep="\t",quote=FALSE)
...
\subsubsection{GAGE}
GAGE was used to generate GO and KEGG pathway analysis using a ranked list analysis (gene counts are taken into consideration)

<<gage,results=hide,echo=F>>=
...
writeGoTables<-function(tissue,category,geneSet,refCols,altCols){
  gage_results <- gage(cnts.matrix, gsets = geneSet, ref = refCols, samp = altCols,  compare ="unpaired", same.dir=TRUE)
  write.csv(gage_results$greater,file=paste("GAGE/GO",tissue,"ant1",category,"up.csv",sep="."),quote=FALSE)
  write.csv(gage_results$less,file=paste("GAGE/GO",tissue,"ant1",category,"down.csv",sep="."),quote=FALSE)
...
go.mf.muscle<-writeGoTables("muscle","molecular_function",go.mf.mm,refMuscleCols,altMuscleCols)
go.cc.muscle<-writeGoTables("muscle","cellular_component",go.cc.mm,refMuscleCols,altMuscleCols)
go.bp.muscle<-writeGoTables("muscle","biological_process",go.bp.mm,refMuscleCols,altMuscleCols)

go.mf.heart<-writeGoTables("heart","molecular_function",go.mf.mm,refHeartCols,altHeartCols)
go.cc.heart<-writeGoTables("heart","cellular_component",go.cc.mm,refHeartCols,altHeartCols)
go.bp.heart<-writeGoTables("heart","biological_process",go.bp.mm,refHeartCols,altHeartCols)

@
\subsubsection{TopGO}
TopGO provides additional tools for exploring enrichment.
...
#requires csv with id field 
#print latex of go summary table
#return GOdata object, table object
getGO<-function(tissue, ontology){
  res<-read.csv(concat(tissue,"Results.csv"))
  #NA's will screw up the KS test
  res<-subset(res,!is.na(padj))
  geneList<-res$pval
  names(geneList)<-res$id
  GOdata <- new("topGOdata", ontology = ontology, allGenes = geneList, geneSel = getSigOrNot, description = "Test", annot = annFUN.org, mapping = "org.Mm.eg.db", ID = "Ensembl")
  resultFisher <- runTest(GOdata, algorithm = "classic", statistic = "fisher")
  resultKS <- runTest(GOdata, algorithm = "classic", statistic = "ks", scoreOrder = "increasing")
  resultKS.elim <- runTest(GOdata, algorithm = "elim", statistic = "ks", scoreOrder = "increasing")
  ...
  tissueGO<-xtable(genTab,type=tex,caption=paste("TopGO top 10 terms of ",tissue," (both up/downregulated)",sep=""),digits=3,display=c('d','s','s','d','d','f','d','g','g','g'))
  print(tissueGO,include.rownames=FALSE, tabular.environment = "longtable", floating = FALSE, size="\\footnotesize")

  return(list(godata=GOdata,gentab=genTab))
}
muscle<-getGO("muscle")
heart<-getGO("heart")
@
